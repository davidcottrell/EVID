...
\begin{abstract}
  \noindent 
  Expansive lines at the polls raise the cost of voting and can precipitate
  unequal treatment of voters.  Despite the importance of lines,
  research on them is hampered by a fundamental measurement problem:
  little is known about the distribution of wait times across voters.
  We argue that early, in-person voter check-in times from
  ...
  of elections that influence the cost of voting, and ultimately,
  whether voters across elections are treated equally.
\end{abstract}
...
Lines outside a polling station reflect administrative decisions about
resource allocation as well as the magnitude of voter turnout
\citep{herronsmith:hanoverstudy}, and among other things we consider
 how voting lines are related to equal treatment.  The connection
between lines and equal treatment is straightforward: if lines
adversely affect some types of voters more than others, then the cost
of voting will vary across voter groups.  This will threaten the
...
Our analysis considers two features of voting lines.  First, waiting in line to vote constitutes a time tax
...
Second, waiting in line to vote may have ``downstream''
...
elections---approximately 58 percent of the voter eligible population turned out in 2012, and 60 percent in
...
consequences---noted above are conceptually distinct. The time tax for
voting could be uniformly spread across voters, which might be
...
concentrated on certain voter groups, violating equal treatment 
regardless of whether there are downstream consequences of waiting to vote.

Notwithstanding post-election surveys of voters asking them about their voting experience, which we discuss
later, little is known about the distribution of individual wait times
across voters in line at the polls. This measurement problem, we argue, is the major
impediment in voting line research.  To address this data limitation, we turn
to a relatively untapped source of electoral information, namely,
early voter check-in times from Florida counties in the 2012 and 2016
General Elections.  The hundreds of thousands of check-in times which
form the basis of our results have several valuable features: they are
not subject to self-reporting biases and check-in time can be linked to a voter's 
race/ethnicity, party registration, gender, age, neighborhood income, and previous voting history. 
% , pieces of information publicly available in Florida's statewide voter file.

In the interests of full disclosure, we emphasize here at the outset 
that the check-in times forming the basis of our research design do
not themselves specify how long a voter waited at the polls before casting a ballot.
Rather, in what follows we offer a set of assumptions under which check-ins can be used to study
voting lines.  Our assumptions are not costless, of course, but they
differ from key assumptions in the extant voting line literature,
namely, that survey respondents accurately remember, and when queried, are able to recount their wait time.  
All research designs have limitations; through triangulation, we argue that the literature on voting lines will be strengthened
to the extent that varied research designs invoke alternative sets of assumptions.
...
Regarding the time tax associated with waiting to vote, as a preview we find a 
...
the more recent presidential election.  This is consistent with national, survey-based
...
three percentage points, consistent to the findings of 
\citet{pettigrew:longlinesminorityprecincts}.  We also find that early voters 
in 2012 who waited in line to vote were less likely to vote early in 2016, \emph{ceteris paribus}.
...
States Supreme Court  in \emph{Reynolds v.\ Sims} (1964),
...
prohibiting the free exercise of the franchise.''\footnote{See
  ...
  (last accessed December 12, 2017) for the text of the decision.} In assessing the 
costs of voting that may debase or dilute the right
...
inevitable anger over long waits at the polls, then-President Barack Obama formed a Presidential Commission in early 2013
...
some of its January 2014, report to long lines, concluding that,
...
surveys query voters about their experiences at the polls, including items on estimated wait times.
...
Cooperative Congressional Election Study (CCES), gauge the relative wait times across states 
as well as overall wait times for different sub-populations of voters.  According to
\citeauthor{stewart:waitingtovote2012}, wait times across the country vary considerably, 
but a state's average wait time tends to be consistent over time. Floridians consistently
report having to endure some of the longest wait times in the country. 
In the 2012 General Election, in-person voters in Florida reported on average waiting 39 minutes to cast a ballot, three times
...
in white locations. In his study of the 2012 General Election,
...
individuals'' (pp.\ 457-458). Alternatively, \cite{herron:confidence} draw on 
an innovative exit poll conducted across Miami-Dade precincts in 2014 
to gauge the experiences of voters as they were leaving their polling stations, a
nd report that those who waited longer in line had less confidence in the electoral process.  

The use of surveys in the study of voter wait times relies on a
key assumption: that self-reports of wait times a voter experienced in a recent
election are accurate.  In light of the literature on memory
distortion, this is a strong assumption. Memory confidence and memory
accuracy can be negatively associated \citep[e.g.,][]{hirstetal:sept11memories}, how an individual thinks
about a given task can be related to the extent to which she thinks about
the passage of time \citep{conti:timeflies}, and one's enjoyment of a
task can depend on feelings about time progression \citep[e.g.,][]{sackettetal:timeflies}.  
In short, it is far from clear that scholars interested in wait times should be optimistic that
...
to vote, and overreporting of turnout is a known problem \citep{ansolhersh:bigdata}.

The point we are making here is that survey-based designs in the study
of voting lines impose some assumptions on the problem they analyze.  The
research design we offer, of course, carries with it its own set of assumptions, and in this way
it is similar to survey-based designs.  However, as will be clear
shortly, our design imposes a very different set of assumptions than
survey-based designs; most notably, it does not risk biases associated
with memory and self-reporting.
...
open records laws make it an excellent laboratory for the study
of election administration, as there is a wealth of information not
only about who votes and by what method of voting, but when and how indivdiuals register to vote or cast ballots.
Drawing on Florida's statewide voter registration records, \cite{herron_smith2013} trace 
the ebb and flow of new voter registrations after the state legislature placed 
...
relocated by local election administrators. In their study of congestion
...
composition. Similarly, by linking multiple statewide voter files and individual-level early voting
...
Through public records requests, we obtained EViD check-in times from six Florida counties, Alachua,
Broward, Hillsborough, Miami-Dade, Orange, and Palm Beach. Table
\ref{tab:evidcounts} lists the corresponding numbers of EViD timestamps.
...
approximately 39 percent and 44 percent of all early in-person ballots cast in
...
surveys that inquire as to when voters cast their ballots.
Moreover, EViD timestamps as associated with official Florida voter
...
Lastly, and very roughly speaking, we observe a flatter, 
more-uniform distribution of check-in times at Karl County Center than
in West Kendall Library.  We draw on this fact later when we try
...
to arrival times.  As we have already note, all data sources and
...
from our EViD data, we do not have voter arrival times, which are not
collected by election officials, or anyone else for that matter.
Indeed, were arrival times regularly tracked by election
administrators across the United States, there would be no need for
the research design employed here, much less national surveys asking about wait times.  
Second, early voters who voted at, say, 9:00am on a given day of early voting may be systematically
different from those who voted at 6:00pm. Consequently, estimating the
effect of voting after closing time---when we know voters must have
waited in line---is potentially confounded by voter-level selection.
Hence, we need to ensure that we control for differences across early
voters as best as we can so that selection into time of early voting
does not confound our estimates of the effect of waiting in 2012 on
future turnout.
...
party registration, gender, age, neighborhood
income, and previous voting history.  And, we make the following
assumption: individuals who voted at Florida early voting locations
which stayed open well past the 7:00pm cutoff time are more likely to
have waited in line than those who voted in locations that closed
before 7:00pm or at this time exactly.  Moreover, we assume that those
who voted just before the 7:00pm cutoff in early voting locations that
...
on turnout and age \citep{costaetal:walkingthewalk}.
Our age estimates in Table \ref{tab:reg} are presumably
underestimated; the regression model that generates them excludes the
youngest voters in 2016, who could not legally register to vote in
Florida in 2008, and younger registered voters are, \emph{ceteris
  paribus}, less likely to turn out to vote
...
best of our knowledge, \citeauthor{pettigrew:longlinesminorityprecincts} is the first scholar
...
turnout and on 2016 turnout are conditional on the hour in which one
voted in 2012.  In both figures, early voters who cast their ballot in the afternoon in 2012 were
more affected by congested polling locations than voters who cast their ballot before noon.
...
because, broadly speaking, EViD check-in times, which are the key to
our research design, are not a panacea in the quest to understand
voting lines and their consequences. These data are useful, but
check-in times do not specify when voters arrived to cast their
ballots. Using check-ins in a line study such as ours thus requires
assumptions, but it is worth noting that a dependence on assumptions
does not make this study unique.

To wit, the existing literature on voting lines relies heavily on
voter self-reports of time spent in line. In other words, and despite
research on memory distortion, this literature assumes that voters can
accurately recall and report how long they waited prior to voting in a
recent election. In contrast, nothing in the results reported on here
requires voters to have accurate recollections of how long they waited
before voting.  We are not arguing that scholars must, or even
should, avoid voter self-reports of time spent in line. Rather, we are
...
estimate any effect of hour of voting cross having voted in a
congested location. Our assumption about late-closing early voting
locations is thus conservative, and we note that we have found
significant effects on future turnout of voters having cast their
ballots in voting locations that we identified as closing late.
...
election officials, policy makers, and voters.  Burdensome wait times not only impose
