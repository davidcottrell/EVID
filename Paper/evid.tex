...
Our analysis considers two features of voting lines.  First, waiting
in line to vote constitutes a time tax \citep{mukherjee:timetax}. This
...
questions associated with all forms of taxation: is the time tax fair
and is the burden of this tax spread uniformly across voters or
concentrated on selected groups?

Second, waiting in line to vote may have ``downstream'' electoral
effects, as characterized by
...
regardless of whether there are downstream consequences of waiting to
vote.
...
of wait times that affect voters. This measurement problem, we argue,
is the most meaningful impediment to voting line research.  To address
this limitation, we turn to a relatively untapped source of electoral
information: early voter check-in times from Florida counties in the
2012 and 2016 General Elections.

The hundreds of thousands of check-in times that form the basis of our
results have several valuable features.  Namely, they are not subject
to self-reporting biases, and each check-in time can be linked to a
single voter's race/ethnicity, party registration, gender, age,
neighborhood income, and previous voting history.  Still, in the
interests of full disclosure we want to emphasize at the outset that a
voter's check-in time by itself cannot specify the length of the line
at the polls in which said voter waited before casting a ballot.
Rather, in what follows we offer a set of assumptions under which
check-ins can be used to assess the prevalence of voting lines.  Our
assumptions are not costless, of course, but they differ from key
assumptions in the extant voting lines literature, which assumes that
voters can accurately remember, and when so queried, can recount their
wait times.  All research designs have limitations.  Through
triangulation, we argue here that the literature on voting lines and
election administration more generally will be strengthened to the
extent that varied research designs invoke alternative sets of
assumptions.
